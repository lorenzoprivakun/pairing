...
`.trim()
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      ...
      const buttonMessage = {
        document: { url: "https://wa.me/+6283137550315" },
...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/d2ef421e66085e5a9ad4a.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'sans', body: wm, sourceUrl: 'https://chat.whatsapp.com/LfpvTsTBnsnKKrs7ZAU4Qr', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
