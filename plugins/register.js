...
let handler = async function (m, { conn, text, usedPrefix }) {
  ...
  age = parseInt(age)
  if (age > 60) throw 'Udah Tua maen WA'
  if (age < 6) throw 'Anda Ngetroll?._.'
  let __waktuh = (new Date - global.db.data.users[m.sender].reglast)
   let _waktuh = (+ 60000- __waktuh)
   let waktuh = clockString(_waktuh)
   if (new Date - global.db.data.users[m.sender].reglast > + 60000) {
